...
export function Field({
  ...
  const focus = accent === "green" ? "focus:border-client-green/60 focus:ring-client-green/15" : "focus:border-grid-blue/60 focus:ring-grid-blue/15";
  return (
    <label className="block">
      <span className="mb-1.5 block text-xs font-medium uppercase tracking-[0.14em] text-white/45">{label}</span>
      <input
        {...props}
        className={`w-full rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3 text-sm text-white outline-none ring-0 transition-all placeholder:text-white/35 focus:ring-4 ${focus}`}
      />
    </label>
  );
}
