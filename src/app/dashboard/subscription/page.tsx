...
function RotatingQuote() {
  ...
  return (
    <div className="relative flex min-h-[4.5rem] items-center justify-center px-4 text-center">
      <AnimatePresence mode="wait">
        <motion.p
          ...
          className="max-w-2xl text-balance text-lg font-medium leading-snug text-white sm:text-2xl"
        >
          {QUOTES[i]}
        </motion.p>
      </AnimatePresence>
  ...
  return (
    <div className="flex flex-col gap-9">
      ...
      <div className="rise" style={{ animationDelay: "40ms" }}>
        <Surface radius="2rem" inner="px-6 py-7">
          <RotatingQuote />
        </Surface>
      </div>

      {/* Free core + current plan */}
      <div className="rise" style={{ animationDelay: "80ms" }}>
        <Surface radius="2rem" inner="p-6 sm:p-7">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <div className="flex items-center gap-2">
                <span className="text-[10px] uppercase tracking-[0.18em] text-white/40">Your plan</span>
                <span className="rounded-full bg-white/10 px-2.5 py-0.5 text-xs font-semibold text-white">{planLabel(plan)}</span>
              </div>
              <p className="mt-2 max-w-md text-sm text-white/55">
                Free always includes the essentials below. {plan === "free" ? "Upgrade any time — cancel any time." : "Thanks for being a member."}
              </p>
            </div>
            {plan !== "free" && (
              <button
      ...
      <p className="rise -mt-2 text-center text-xs text-white/45" style={{ animationDelay: "150ms" }}>
      ...
      <div className="rise" style={{ animationDelay: "180ms" }}>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {GATED_FEATURES.map((f) => {
            ...
            const tier = tierById(f.plan);
            return (
              <div
                key={f.key}
                className="flex items-center gap-3 rounded-2xl border border-white/8 bg-white/[0.025] p-4"
              >
                <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white/[0.06] ring-1 ring-white/10 ${tier?.accent ?? "text-white/70"}`}>
                  <Icon name={f.icon} size={19} />
                </span>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1.5">
                    <span className="truncate text-sm font-medium text-white">{f.label}</span>
                    <span className="shrink-0 rounded-full bg-white/8 px-1.5 py-0.5 text-[9px] uppercase tracking-wider text-white/50">{tier?.name}</span>
                  </div>
