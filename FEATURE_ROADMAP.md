...
|---|---|---|---|
| **0. Supabase foundation** | Provision Supabase (EU); migrate DB Turso→Postgres (Drizzle kept); Better Auth→Supabase Auth (login/signup UI preserved); RLS on every table; env/secrets; install dormant SDKs as needed | **Approved — not started** | Supabase project + creds (SETUP.md §1); confirm Turso data disposable |
