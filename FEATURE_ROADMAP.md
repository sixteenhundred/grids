...
| **2. Platform persistence + gating** | Real tables; single **server-side entitlements service**; DB-backed CMS/config; **50/5 GB** quotas; remove demo auth | **Foundation ✓** — server `entitlements.ts` (subscription→feature access, Stripe-webhook integration point), `subscription`/`usage`/`app_config` tables (RLS-locked), `config-store` CMS, `quota.ts` (50 GB total / 5 GB file, atomic). **Supabase Storage ✓** — private `uploads` bucket + owner-scoped storage RLS (`uploads_own`, files keyed `${uid}/…`); `storage.service.ts` (server upload reserves quota then rolls back on failure, signed up/download URLs) verified headless (upload/list/delete + atomic 50/5 GB guard blocks over-cap; throwaway user cascades clean). **Remaining:** thread server entitlements into the live nav/feature gates; replace data-URL uploads in shop/transfer UIs with the storage service; real booking/contract/review/dispute tables; remove demo-mode at launch. | decision (3) empty-states-at-launch OK |
