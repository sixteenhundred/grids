...
|---|---|---|---|
| **0. Supabase foundation** | Provision Supabase (EU); migrate DB Turso→Postgres (Drizzle kept); Better Auth→Supabase Auth; RLS on every table; env/secrets | **DB migrated ✓** + **Auth swapped to Supabase Auth ✓** (proxy session refresh, `auth.users`→`public.user` mirror trigger, guest = anonymous sign-in; verified headless: create→mirror→sign-in→delete). **Remaining:** RLS on every table + isolation tests. | Live guest/signup needs 2 Supabase toggles (Anonymous ON, Confirm-email OFF). Prod deploy needs the IPv4 **pooler** string. |
