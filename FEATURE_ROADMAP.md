...
|---|---|---|---|
| **0. Supabase foundation** | Provision Supabase (EU); migrate DB Turso→Postgres (Drizzle kept); Better Auth→Supabase Auth (login/signup UI preserved); RLS on every table; env/secrets | **DB migrated ✓** (Postgres schema + postgres-js client + Drizzle config + Better-Auth-on-pg + 15 tables pushed to Supabase + build green). **Remaining:** Supabase Auth swap, RLS. | Auth swap + RLS need `service_role` key + JWT secret + IPv4 **pooler** string (SETUP.md §1) |
