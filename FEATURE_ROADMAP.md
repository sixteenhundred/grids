...
| **2. Platform persistence + gating** | Real tables; single **server-side entitlements service**; DB-backed CMS/config; **50/5 GB** quotas; remove demo auth | **Foundation ✓** — server `entitlements.ts` (subscription→feature access, Stripe-webhook integration point), `subscription`/`usage`/`app_config` tables (RLS-locked), `config-store` CMS, `quota.ts` (50 GB total / 5 GB file, atomic). **Remaining:** migrate client-side gating → server entitlements; Supabase Storage uploads + quota wiring; real booking/contract/review/dispute tables; remove demo-mode at launch. | decision (3) empty-states-at-launch OK |
