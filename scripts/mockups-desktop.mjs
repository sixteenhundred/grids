/**
 * Desktop mockup generator — 1440x900 @2x PNGs of every page, saved to mockups/.
 * Accepts the cookie banner first so shots are unobstructed (full screen).
 * Run the dev server first (npm run dev), then: node scripts/mockups-desktop.mjs
 */
import puppeteer from "puppeteer";
import { mkdirSync } from "node:fs";

const BASE = process.env.MOCKUP_BASE || "http://localhost:3000";
const OUT = "/Users/johnhope/grids/mockups/";
...
await page.setViewport({ width: 1440, height: 900, deviceScaleFactor: 2 }); // desktop @2x
